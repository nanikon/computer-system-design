...
Tick* define_color(char color, uint8_t* new_color, Tick** other_buf, Mode* mode){
  Tick* buffer;
  switch (color){
    case 'r':
      *new_color = RED_COLOR;
...
void fill_tick_array(char color, uint8_t brightness, uint8_t duration, int8_t softness, Mode* mode){
  uint8_t new_color;
  Tick* other_buffer;
